...
|---|--------|------|---------|------|-------------|
| 1 | **P0** | L7 | B3/S3 基于**笔**级别突破回踩，且仅检查最后一个中枢 | B3/S3 信号过多（尤其 30 分钟级别） | 应基于**段**级别突破，且检查所有中枢 |
| 2 | **P1** | L2 | 反方向分型不满足笔规则时，丢弃原始起始分型（`stroke.py:141`） | 震荡行情笔可能缺失，向上传导导致段偏少 | 应保留原起始分型继续等待，仅同向更极端时才更新 |
| 3 | **P1** | L7 | B2/S2 未限制在 B1 后**第一次**回调 | B2 信号可能出现在远离实际二买的位置 | B2 仅在 B1 后首次回调不创新低时产生，产生一次 |
