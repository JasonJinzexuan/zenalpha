...
import type { AnalysisResult, WalkState, Signal } from '@/types/chan'
...
export default function OverviewPage() {
  ...
  const runScan = useCallback(async () => {
    setLoading(true)
    try {
      const res = await scanInstruments({ instruments, level: '1d', limit: 250 })
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <StatCard label="紧急" value={urgentRows.length} color="text-accent-red" />
        <StatCard label="关注" value={watchRows.length} color="text-accent-yellow" />
      </div>
      ...
      {urgentRows.length > 0 && (
        <div className="card border-accent-red/30">
          ...
          <div className="divide-y divide-bg-border">
            {urgentRows.map(({ item }) => (
              <SignalAlertRow key={item.instrument} item={item} onNavigate={s => nav(`/nesting-map/${s}`)} />
            ))}
          </div>
        </div>
      )}
              ...
              {rows.map(({ item, status }) => {
                ...
                return (
                  <tr
                    ...
                    onClick={() => nav(`/chart/${item.instrument}`)}
                  >
                    ...
                    <td className="px-2 py-3">
                      <button
                        onClick={e => { e.stopPropagation(); nav(`/nesting-map/${item.instrument}`) }}
