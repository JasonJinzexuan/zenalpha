...
| 9 | L7 | B3/S3 改为段级别突破 + 检查所有中枢 + 每中枢仅首次 + 严格不等式 | 第 020 课 |
| 10 | L7 | B2/S2 限制在 B1/S1 后首个反向段回调 | 第 053 课 |
| 11 | L2 | 保留原始起始分型（反向不满足规则时不丢弃） | 第 062 课 |
| 12 | L7 | 信号按 (type, timestamp) 去重累积，非每轮覆盖 | — |
| 13 | L8 | 替换加权评分为区间套递进定位 | 第 030 课 |

#### 已知局限 & 待改进

| # | 层级 | 问题 | 影响 |
|---|------|------|------|
| 1 | L6 | stagnation 与 area divergence 高度相关，三条件实质为两条件 | 背驰判断偏松 |
| 2 | L1 | 分型替换（同类型更极端）不通知 StrokeBuilder 撤回旧分型 | 边缘情况笔端点可能非最极端 |
